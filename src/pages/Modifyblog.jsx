...
const Modifyblog = () => {
    ...
    const {edit} = useSelector(state=>state.blogs)
    ...
    const {title, description, author, image, isPublished}=input;
    ...
    const handlechange = (e)=>{
      ...
      const handlesubmit = (e)=>{
        e.preventDefault();
        if (edit.editMode) {
          dispatch(updatefetchTodo({_id:edit.blog._id, title, description, author, image, isPublished}));
          alert("data updated")
          if(isSuccess){
            navigate("/blogs")
          }
  ...
  return (
    <>
     <form onSubmit={handlesubmit}>
      ...
      <TextField name='image' value={image} onChange={handlechange} margin='normal' variant='outlined'/>
